"""Add an `icon` column to group_types and food_preferences (Azure SQL) and
backfill emoji icons — matching the `activities` table shape.

Schema change (per table):
    icon  NVARCHAR(20)  NULL

Run from project root (needs an authenticated Azure session — `az login`):
    venv/bin/python migrations/add_icons_to_group_food_prefs.py
